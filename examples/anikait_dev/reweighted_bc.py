...
def main(_):
    ...
    gold_model = rbc_trainer.accelerator.prepare_model(gold_model)
    gold_model.eval()
    ...
    if batch_size_online_data == 0:
        ...
        zipped_dataloaders = rbc_trainer.dataloader
    else:
        zipped_dataloaders = zip(rbc_trainer.dataloader, pref_dataset_dataloader)
        
    if FLAGS.use_gold_reward_model:
        rew_fn = lambda x: get_gold_score(completions=x, num_shards=FLAGS.num_gold_shards)
    else:
    ...
    for epoch, batches in tqdm(enumerate(zipped_dataloaders)):
        ...
        pref_query_tensors = accelerate.utils.send_to_device(pref_query, rbc_trainer.accelerator.device)
        ...
        pref_response_tensors = accelerate.utils.send_to_device(pref_response_tensors, rbc_trainer.accelerator.device)
